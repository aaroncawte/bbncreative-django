...
class ProjectSerializerTests(TestCase):
    def test_project_serializer_representation(self):
        ...
        self.assertEquals(
            representation["progress"]["date"], datetime.date.today().isoformat()
        )
        ...
        self.assertEquals(representation["callToAction"]["label"], "Go to Website")
        self.assertEquals(
    ...
    def test_customised_project_serializer_representation(self):
        project = Project.objects.create(
            name="Test Project 2", is_complete=True, date_complete="2020-04-01"
        )
    ...
    def test_project_with_images_serializer_representation(self):
        ...
        project = Project.objects.create(icon=image_mock, hero=image_mock)

        representation = ProjectSerializer().to_representation(project)

        self.assertEquals(representation["images"]["icon"][-14:], "mocked_img.jpg")
